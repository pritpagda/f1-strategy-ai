...
async def get_strategy_recommendation(request: StrategyRequest):
    try:
        result = get_strategy_recommendation_from_llm(current_lap_data=request.current_lap_data,
                                                      stint_history=request.stint_history,
                                                      race_progress=request.race_progress)
        return {"recommendation": result.get("recommendation", "No recommendation generated."),
                "reasoning": f"AI model: {result.get('reasoning', 'No reasoning provided.')}",
                "confidence": result.get("confidence", "medium")}
    except Exception as e:
        return {"recommendation": "Strategy generation failed.", "reasoning": f"LLM error: {e}", "confidence": "low"}
