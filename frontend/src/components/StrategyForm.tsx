...
import {BarChart3, Loader2, Plus, Timer, Zap} from 'lucide-react';
...
const defaultCurrentLapData: CurrentLapData = {
    lap_number: 1, compound: '', tyre_life: 0, lap_pace: 0, driver: '', stint: 1,
};

const defaultStintLapData: StintLapData = {
    lap: 1, compound: '', tyre_life: 0, lap_pace: 0, track_status: 1,
};
...
const StrategyForm = ({onStrategyResult, onShowModal, apiBaseUrl}: StrategyFormProps) => {
    ...
    }, {...defaultStintLapData, lap: 2}, {...defaultStintLapData, lap: 1},]);
...
<<<<<<< HEAD
    const handleSubmit = async (e: FormEvent) => {
        e.preventDefault();
        if (!currentLapData.compound || !currentLapData.driver || raceProgress < 0 || raceProgress > 1) {
=======
...
<<<<<<< HEAD
        try {
            const payload: StrategyData = {
                ...
                stint_history: stintHistory.filter(sh => sh.compound && sh.lap > 0),
                race_progress: raceProgress,
            };

            const response = await api.post('/strategy', payload);
            const responseData = response.data;
            if (responseData.recommendation && responseData.reasoning && responseData.confidence) {
                onStrategyResult({
...
<<<<<<< HEAD
    return (<form onSubmit={handleSubmit} className="space-y-4 p-3 rounded-lg">
...
>>>>>>> f573c20f5619dc5a654373599aa9deb4b5c7eac1
            ...
            {stintHistory.length === 0 && (
                <p className="text-gray-400 italic text-center text-sm">No stint laps added yet. Click "Add Lap" to
                    begin.</p>)}
            {stintHistory.map((stint, idx) => (<div
                key={idx}
                className="mb-3 grid grid-cols-1 md:grid-cols-5 gap-3 bg-black/30 p-3 rounded-md border border-gray-700 last:mb-0 transform transition-transform duration-300 hover:scale-[1.005] hover:border-red-600"
            >
                <div>
