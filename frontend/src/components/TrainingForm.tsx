...
const TrainingForm: React.FC<TrainingFormProps> = ({
  ...
  <form onSubmit={onSubmit} className="space-y-4 p-3 rounded-xl">
    <div className="relative">
      <label htmlFor="year" className="block text-gray-300 font-semibold text-sm mb-1">
        Race Year
      </label>
      <input
        id="year"
    ...
    <div className="relative">
      <label htmlFor="race_name" className="block text-gray-300 font-semibold text-sm mb-1">
        Race Name
      </label>
      <input
        id="race_name"
    ...
    <div className="pt-3">
      <button
        type="submit"
        disabled={isTraining}
        className={`w-full flex items-center justify-center gap-2 py-2 px-4 rounded-full font-extrabold text-sm transition-all duration-300 transform ${
          isTraining
