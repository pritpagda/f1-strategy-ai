...
const TrainingPage: React.FC = () => {
    ...
    return (
        <div
            className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-red-900 text-white p-4 overflow-y-auto">
            <div className="container mx-auto px-4 py-6">
                <div className="max-w-3xl mx-auto text-center mb-8">
                    ...
                    <p className="text-base text-gray-300 max-w-xl mx-auto leading-relaxed">
                        Prepare the prediction model by training it with historical race data. Select a specific race
                        and year to fine-tune the AI's understanding of track conditions, driver performance, and race
                        dynamics.
                    </p>
                </div>
                <div
                    className="max-w-2xl mx-auto bg-black/60 backdrop-blur-lg border-t-4 border-red-600 rounded-2xl shadow-2xl p-6 transform transition-transform duration-500">
                    ...
                    {resultMessage.status && (
                        <div
                            className={`mt-4 text-center rounded-xl px-4 py-2 font-bold text-sm transition-opacity duration-500 ${resultMessage.status === 'success' ? 'bg-green-700/80 text-white' : 'bg-red-700/80 text-white'} opacity-100`}>
                            {resultMessage.text}
                        </div>
                    )}
                </div>
                <div className="mt-12">
                    ...
                    <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
                        <FeatureButton Icon={Home} title="Home"
                                       description="Return to the main dashboard with all AI racing features."
                                       onClick={() => navigate('/')}/>
                        <FeatureButton Icon={Rocket} title="Predict"
                                       description="Forecast lap times, pit stop windows, and race outcomes with high accuracy."
                                       onClick={() => navigate('/predict')}/>
                        <FeatureButton Icon={BarChart3} title="Analyze"
                                       description="Dive into advanced AI-driven race strategy analysis and simulations."
